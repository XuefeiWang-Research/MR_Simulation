setwd('/home1/WangXF/MRsimulation/BothNeg/')
    ...
    for (pleio_effect in pleio_effects) {  # 遍历多效性效应大小
      for (N_Eff_SNP_PhenoAB in pleio_snp_counts) {  # 遍历多效性SNP数量
        ...
        if (current_combination %% 10 == 0) {
          write.csv(results, "statistical_power_BothNeg1000_temp.csv", row.names = FALSE)
...
# 保存最终结果
write.csv(results, "statistical_power_BothNeg1000.csv", row.names = FALSE)
cat("\nAll complete sensitivity analysis finished! Results saved to statistical_power_BothNeg1000.csv\n")
...
# 显示保存文件总结
cat("\n=== File Saving Summary ===\n")
cat("Main results CSV file: statistical_power_BothNeg1000.csv\n")
