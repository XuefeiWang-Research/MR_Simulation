setwd('/home1/WangXF/MRsimulation/BothPos/')
...
run_simulation <- function(N_Ind_Train, N_Ind_Test, AtoB, pleio_effect, N_Eff_SNP_PhenoAB, N_Sim=1000, seed=73, folder_path=NULL, test_folder_path=NULL) {
    ...
    for (pleio_effect in pleio_effects) {  # 遍历多效性效应大小
      for (N_Eff_SNP_PhenoAB in pleio_snp_counts) {  # 遍历多效性SNP数量
        ...
        sim_result <- run_simulation(N_Ind_Train, N_Ind_Test, AtoB, pleio_effect, 
                                     N_Eff_SNP_PhenoAB, N_Sim=1000, seed=73, 
        ...
        if (current_combination %% 10 == 0) {
          write.csv(results, "statistical_power_BothPos1000_temp.csv", row.names = FALSE)
...
# 保存最终结果
write.csv(results, "statistical_power_BothPos1000.csv", row.names = FALSE)
cat("\nAll complete sensitivity analysis finished! Results saved to statistical_power_BothPos1000.csv\n")
...
# 显示保存文件总结
cat("\n=== File Saving Summary ===\n")
cat("Main results CSV file: statistical_power_BothPos1000.csv\n")
